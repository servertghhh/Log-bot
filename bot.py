...
DEV_GITHUB = "https://youtube.com/@thenoobcoder01?si=UQWh6t7oruh36T6D"
...
def get_permanent_keyboard(user_id=None):
    ...
    keyboard.add(
        KeyboardButton(f"{EMOJI['refer']} রেফার"),
        KeyboardButton(f"{EMOJI['latest']} লেটেস্ট")
    )
    keyboard.add(
        KeyboardButton(f"{EMOJI['developer']} ডেভলপার"),
...
def extract_credentials(text):
    """শুধু ইউজারনেম:পাসওয়ার্ড বের করে - MAX ১০টা"""
    lines = text.split('\n')
    credentials = []
    ...
    for line in lines:
        ...
        if ':' in line:
            ...
            if len(parts) >= 2:
                ...
                if username and password and len(username) > 1 and len(password) > 0:
...
def call_api(url=None):
    """API থেকে ডেটা আনে - স্ট্রিমিং করে"""
    try:
        params = {}
        if url:
            params['url'] = url
...
def start_command(message):
    ...
    if not user:
        ...
        if not check_channel_membership(user_id):
            welcome_text = f"""
{EMOJI['sparkle']} <b>WELCOME {first_name}!</b>
...
{coin_display(user_id)}
    ...
    bot.reply_to(message, 
        f"{EMOJI['sparkle']} WELCOME {first_name}!\n{coin_display(user_id)}",
        parse_mode='HTML', reply_markup=get_permanent_keyboard(user_id))
...
def callback_handler(call):
    ...
    if call.data == "verify_me":
        if check_channel_membership(user_id):
            ...
            bot.answer_callback_query(call.id, "বাইনসোদ আগে চ্যানেল জয়েন কর 😒🫵", show_alert=True)
...
def main():
    ...
    print("✅ বট চালু!")
    print(f"📡 API: {API_URL}")
    print("📌 শুধু প্রথম 500KB ডাটা নেওয়া হবে")
